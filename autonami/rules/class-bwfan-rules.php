	...
	class BWFAN_Rule_Forminator_Form_Field extends BWFAN_Rule_Base {
		...
		public function conditions_view() {
			...
			$value_args = array(
				...
				'choices'     => $values,
				'class'       => 'bwfan_field_one_half bwfan_forminator_form_fields',
		...
		public function ui_view() {
			?>
            Form Field
            '<%= bwfan_events_js_data["forminator_form_submit"]["selected_form_fields"][condition['key']] %>' <% var ops = JSON.parse('<?php echo wp_json_encode( $this->get_possible_rule_operators() ); ?>'); %>
