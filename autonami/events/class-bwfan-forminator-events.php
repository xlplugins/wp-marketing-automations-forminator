<?php
if ( ! class_exists( 'BWFAN_FORMINATOR_Form_Submit' ) ) {
	final class BWFAN_FORMINATOR_Form_Submit extends BWFAN_Event {
		...
		private function __construct() {
			$this->event_merge_tag_groups = array( 'forminator_forms' );
			$this->event_name             = __( 'Form Submits', 'wp-marketing-automations' );
			$this->event_desc             = __( 'This event runs after a form is submitted', 'wp-marketing-automations' );
			...
			$this->optgroup_label         = __( 'Forminator Forms', 'wp-marketing-automations' );
			$this->priority               = 10;
		...
		public function process( $form_id ) {
			...
			$form_title         = function_exists( 'forminator_get_form_name' ) ? forminator_get_form_name( $form_id ) : '';
			$data['form_title'] = ! empty( $form_title ) ? $form_title : get_the_title( $form_id );
